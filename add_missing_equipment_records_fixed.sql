...
-- Using only the columns that actually exist in the equipment table

-- Step 1: Insert missing equipment records from equipment_risk_assessment
-- Based on the screenshot, equipment table has: 設備ID, 設備名, 設備種別ID, 設備タグ, 設備場所, 製造者, 型式, 設置年月日, 稼働状態, 重要度, created_at, updated_at
...
SELECT 'Equipment from risk assessment:' as info, COUNT(*) FROM equipment WHERE 設備名 LIKE 'TK%' OR 設備名 LIKE 'HX%' OR 設備名 LIKE 'PU%';
